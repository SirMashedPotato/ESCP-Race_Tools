...
namespace ESCP_RaceTools
{
     ...
     * This whole thing disgusts me and should really be redone
     */
    ...
    {
        [HarmonyPrefix]
        public static bool Prefix(IdeoFoundation_Deity.Deity deity, ref IdeoFoundation_Deity __instance)
        {
            if (__instance.ideo.StructureMeme != null)
            {
                if (ESCP_RaceTools_ModSettings.IdeologyDivinesNames)
                {
                    ...
                    IdeoCultureProperties cultureProps = IdeoCultureProperties.Get(__instance.ideo.culture);
                ...
                if (__instance.ideo.StructureMeme.defName == "ESCP_Structure_OriginDaedricCult")
                {
                    IdeoOrginProperties props = IdeoOrginProperties.Get(__instance.ideo.StructureMeme);
                    if (props != null)
                    {
                        int index = Rand.RangeInclusive(0, props.iconPath.Count - 1);
                        deity.iconPath = props.iconPath[index];
                        deity.name = __instance.ideo.StructureMeme.fixedDeityNameTypes[index].name;
                        deity.type = __instance.ideo.StructureMeme.fixedDeityNameTypes[index].type;
                        Gender supremeGender = __instance.ideo.SupremeGender;
                        if (supremeGender != Gender.None)
                        {
                            deity.gender = supremeGender;
